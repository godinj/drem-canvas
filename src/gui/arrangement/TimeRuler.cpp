...
void TimeRuler::paint (juce::Graphics& g)
{
    ...
    double endTime = startTime + static_cast<double> (getWidth()) / pixelsPerSecond;
    ...
    for (double t = firstTick; t <= endTime; t += tickInterval)
    {
        ...
        float x = static_cast<float> ((t - startTime) * pixelsPerSecond);
        ...
        if (tickInterval >= 5.0)
        {
            ...
            {
                float mx = static_cast<float> ((minor - startTime) * pixelsPerSecond);
